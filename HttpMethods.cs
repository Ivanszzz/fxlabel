...
namespace AHLabelPrint
{
    public class HttpMethods
    {
		...
		{
			//DBC.ErrLog.Log("url=" + url + ",body=" + body + ",bearerToken=" + bearerToken);

			HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(url);

            httpWebRequest.Headers.Add("Cookie", Program.Cookies);
			...
			try
			{
                using (HttpWebResponse httpWebResponse = (HttpWebResponse)httpWebRequest.GetResponse())
                {
                    ...
                    string setcookies = httpWebResponse.Headers.Get("Set-Cookie");
                    if (!string.IsNullOrEmpty(setcookies))
                    {
                        ...
                        {
                            Program.Cookies = seesionidRegex.Match(setcookies).Value;
                        }
                    }
        ...
        #region DownFile
        public static string HttpDownload(string serverFileUrl,string clientSaveFolder)
        {
            try
            {
                string fileName = Path.GetFileName(serverFileUrl);
                string filePath = clientSaveFolder + "\\" + fileName;
                ...
                {
                    // 设置参数
                    HttpWebRequest request = WebRequest.Create(serverFileUrl) as HttpWebRequest;
                    //发送请求并获取相应回应数据
